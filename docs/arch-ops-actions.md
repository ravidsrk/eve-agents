...
## Human-owned gates
- **BASE → main promotion:** DONE — `ravidsrk/adversarial-fresh` merged via PR #9.
- **Production deploy:** merge ≠ deploy; rotate/set secrets before relying on live webhooks.
...
- **Code:** Unauthenticated / wrong-secret POSTs return 401; `scripts/smoke-alert-webhook.sh` and `evals/webhook-alert.eval.ts` assert this.
